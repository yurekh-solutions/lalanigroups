...
const ProjectDetail = () => {
  ...
  // Use the exact full address for map search - this is most reliable
  const mapAddress = project?.address || `${project?.name}, ${project?.location}, Mumbai, India`;
  
  // Always use address search with highest zoom to show the building
  const mapUrl = `https://www.google.com/maps?q=${encodeURIComponent(mapAddress)}&z=19&output=embed`;
  const mapSearchUrl = `https://www.google.com/maps?q=${encodeURIComponent(mapAddress)}&z=19`;
